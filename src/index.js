...
const App = {
 ...
 renderProductDetails: async function(productId) {
  const { getProduct, escrowInfo } = this.instance.methods;
  ...
  if(p[8] == '0x0000000000000000000000000000000000000000') {
    ...
    $("#buy-now").hide();
    var i = await escrowInfo(productId).call()
    $("#buyer").html(i[0]);
    $("#seller").html(i[1]);
    $("#arbiter").html(i[2]);
 ...
 renderProduct: async function(index) {
  ...
  node.append("<img width='200' height='100' src='http://localhost:8080/ipfs/"+f[3]+"' />");
  node.append("<div class='title'>" + f[1] + "</div>");
  node.append("<div> Registration Fees: " + displayPrice(f[6]) + "</div>");
